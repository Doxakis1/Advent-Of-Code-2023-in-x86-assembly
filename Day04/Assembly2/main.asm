; For part two I used the code from day one but changed the final part to store the line winnings in an array
...
section .bss
    ...
    sum     resd 1
    index   resd 1 ; holds current winnings index
    g_sum   resd 1 ; game sum
    buffer  resb BUFFER_SIZE ; I can increase if needed
    nums_a  resd BUFFER_SIZE ; holds winings
    nums_b  resd BUFFER_SIZE ; holds copies count
section .text
    global _start
...
_init_winnings_loop:
    cmp ecx, BUFFER_SIZE
    jge _repeat
    lea esi, [nums_a + ecx * 4]
    mov [esi], dword -1
    lea esi, [nums_b + ecx * 4]
    mov [esi], dword -1
    inc ecx
    jmp _init_winnings_loop
_repeat:
    mov ecx, [esp + 4]
...
_add_wins:
    mov ebx, [esp + 4]
    mov eax, [index]
    lea edi, [nums_a + eax * 4] ; ticket index
    mov [edi], dword 1
    lea esi, [nums_b + eax * 4] ; ticket wins
    mov [esi], dword 0
_increased_i:
    inc eax
    mov [index], eax ; update counter
    cmp ebx, 0
    jle _repeat
    mov [esi], ebx
    mov eax, 1
...
_increase_nexts:
    ...
    dec ebx
    jmp _increase_nexts
_add_copies_loop_inc:
    mov ecx, dword [esp]
    inc ecx
    mov [esp], ecx
    jmp _add_copies_loop
